...
#include "UsbDevDef.h"//Eric 20150905—1
static UINT32  uiKeycount =0;
static  BOOL    uiKeypress =FALSE;
static  BOOL    uiPowerupstat =TRUE;
extern BOOL g_bWifiEscStaus ;//eric edit 0909-3
extern UINT32      gERICcount;
 BOOL     gWifiStaus=TRUE;
...
INT32 UIFlowWndMovie_RecModeTrig(VControl *, UINT32, UINT32 *);//eric@20150821-低帧录影
...
EVENT_ITEM(NVTEVT_RECORDING_MODE_TRIG, UIFlowWndMovie_RecModeTrig)//eric@20150821
 ...
 BOOL   CheckStorageErr = FALSE;
    ...
    case NVTEVT_KEY_PRESS:
        ...
        case MOV_ST_REC|MOV_ST_ZOOM: // Isiah, implement YUV merge mode of recording func.
		   ...
		   UIFlowWndMovie_SetGsensorTrigFlag(FALSE);    //ERIC EDIT 20150905            
                //FlowMovie_IconHideLock(); //eric 和后面 FlowMovie_UpdateIcons(TRUE);重复
                UISound_Play(DEMOSOUND_SOUND_KEY_TONE);
                // update ui window icon
  ...
  // if(GxCustom_IsDCPowerON())
    ...
    else
	{
     ...
     debug_msg("^G ===========gERICcount =%d\r\n",gERICcount);
  	     ...
  	     {
	       firstpoweron=FALSE; 
	        if(SysGetFlag(FL_WIFI)==WIFI_OFF)
	   	  {
	   	
	          Ux_PostEvent(NVTEVT_KEY_SHUTTER2, 1, NVTEVT_KEY_PRESS);
		   debug_msg("^G ===========SysGetFlag(FL_WIFI)==WIFI_OFF\r\n");
	   	  }  
	      }//Eric 20150905—1 end
	     else
	     {
...
#if 0
 if(firstpoweron==TRUE)   //Eric 20150905—1 begin
   {
	    	     firstpoweron=FALSE; 
	    	   ...
	    	   //SxTimer_SetFuncActive(SX_TIMER_DET_GSENSOR_ID,TRUE);	//本CODE已经开启	 
  }//Eric 20150905—1 end
  #endif
  ...
  debug_msg("Eric****************eric\r\n");	  
	...
	case MOV_ST_REC:
		  if(g_bgsensor==FALSE)
		     {
			g_bgsensor = TRUE;     
		         FlowMovie_IconDrawLock();//显示LOGO
	                MediaRec_SetCrash();//加锁
		     }	
	        break;		
        }
        break;
...
INT32 UIFlowWndMovie_OnKeyShutter2(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    //debug_ind(("[UIFlowWndMovie_OnKeyShutter2][%d]*****eric\n\r", uiKeyAct));
    debug_msg("UIFlowWndMovie_OnKeyShutter1: %x,%x\n\r",uiKeyAct,NVTEVT_KEY_CONTINUE);
   #if 1
     if(uiKeyAct==NVTEVT_KEY_CONTINUE)
    {
	 ...
	 debug_msg("UIFlowWndMovie_OnKeyShutter2*********************************ERIC_DEBUG\n\r");//ERIC_DEBUG
   return UIFlowWndMovie_OnExeRecord(pCtrl,paramNum,paramArray);
    }
#endif
     //return UIFlowWndMovie_OnExeRecord(pCtrl,paramNum,paramArray);
...
INT32 UIFlowWndMovie_OnMovieOneSec(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    case MOV_ST_REC|MOV_ST_ZOOM:
		...
		///ericif(UxCtrl_IsShow(&UIFlowWndMovie_PanelCtrl))
			///ericGxLED_SetCtrl(KEYSCAN_LED_GREEN,TURNON_LED,TRUE);
		///ericelse
			///ericGxLED_SetCtrl(KEYSCAN_LED_GREEN,TURNON_LED,FALSE);
        if (paramNum)
        {
...
INT32 UIFlowWndMovie_OnMovieFull(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    FlowMovie_StopRec();
    FlowMovie_IconHideLock();//eric 2015 08 22
...
{
  if(g_bWifiEscStaus==TRUE)//eric edit 0909-3
     	{
	  g_bWifiEscStaus = FALSE;//eric edit 0909-3
       }//eric edit 0909-3
  else{//eric edit 0909-3
    //FlowMovie_StopRec();
    KeyScan_EnableMisc(TRUE);
    //gMovData.State = MOV_ST_VIEW;
    gMovData.State= MOV_ST_WARNING_MENU;
    Ux_OpenWindow(&UIFlowWndWrnMsgCtrl,2,UIFlowWndWrnMsg_StatusTXT_Msg_STRID_CARD_FULL,FLOWWRNMSG_TIMER_2SEC);//eric 05
    FlowMovie_IconDrawMaxRecTime(&UIFlowWndMovie_Static_maxtimeCtrl);
  	}//eric edit 0909-3
    return NVTEVT_CONSUME;
}
...
UINT32 day = 0, month = 0, year = 0, hour = 0, min = 0, sec = 0;
	...
	UINT32 Days;
       RTC_DATE rtcDate;
   	//if (SysGetFlag(FL_MOVIE_GPS)==GPS_ON)
   	//{
      		GPSRec_GetRMCDate(&RMCInfo);
...
INT32 UIFlowWndMovie_OnTimer(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    case NVTEVT_05SEC_TIMER:
        ...
        /*else
            {
            if(gMovData.State==MOV_ST_VIEW  &&Startrec)
                {
                Startrec = FALSE;
                Ux_PostEvent(NVTEVT_KEY_SHUTTER2, 1, NVTEVT_KEY_PRESS);
                }
       ...
       /* if(!MediaRec_IsRecording())
        ...
        #if 1//(AUTO_WIFI==ENABLE)
	  if (uiPowerupstat ==TRUE)//eric edit begin
	  	{
	  	   uiPowerupstat =FALSE;
	        //if ((DxUSB_GetIsUSBPlug())&&(SysGetFlag(FL_MOVIE_PARKING) != MOVIE_PARKING_ON))//Eric 20150905—2
	   	{
	   		if(SysGetFlag(FL_WIFI)==WIFI_ON)//eric edit begin
         			{
            //static BOOL autoWifi = FALSE;
        ...
        if (SysGetFlag(FL_MOVIE_MOTION_DET) == MOVIE_MOTIONDET_ON)
        {
            ...
            if (MD_Process()==TRUE)
            {
               ...
               //if (uiMotionDetStop>=60) // 30 Sec// if (uiMotionDetStop>=20) // 10 Sec
                if (uiMotionDetStop>=20) // 30 Sec// if (uiMotionDetStop>=20) // 10 Sec
                {
                    uiMotionDetStop = 0;
                    switch(gMovData.State)
                    {
