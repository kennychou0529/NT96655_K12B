...
static KEY_OBJ g_KeyTable[] =
{
    ...
    {FLGKEY_MODE,         KEY_CONTINUE,   NVTEVT_KEY_MODE,          NVTEVT_KEY_CONTINUE,  0},//eric edit
    {FLGKEY_MODE,          KEY_RELEASE,     NVTEVT_KEY_MODE,          NVTEVT_KEY_RELEASE, 0},//eric edit
...
void UI_DetCustomerModeKey(void)
{
	...
	static UINT32 uiModeKeyPressCount = 0;
       //eric edit begin
	static UINT32 uiOkKey    = KEYSCAN_CUSTOMER_UNKNOWN;
	static UINT32 uiOkState    = KEYSCAN_CUSTOMER_INIT_STATE;
	static UINT32 uiOkKeyPressCount = 0;
      //eric edit end
	...
	if((uiADCValue>=(KEY_ADC_MODE-KEY_ADC_RANGE))&&(uiADCValue<(KEY_ADC_MODE+KEY_ADC_RANGE)))
	{
        ...
        {
            //debug_msg("uiModeKey == KEYSCAN_CUSTOMER_PRESSED*********************************ERIC_DEBUG\n\r");//ERIC_DEBUG
            ...
            if(uiModeState == KEYSCAN_CUSTOMER_PRESS_STATE)
            {
		     ...
		     uiModeState    = KEYSCAN_CUSTOMER_INIT_STATE;			
                //UISound_Play(DEMOSOUND_SOUND_KEY_TONE);							
                  ...
                  //Ux_PostEvent(NVTEVT_KEY_SHUTTER2, 1, NVTEVT_KEY_CONTINUE);
		    debug_msg("uiModeState == KEYSCAN_CUSTOMER_PRESS_STATE*********************************ERIC_DEBUG\n\r");//ERIC_DEBUG
	...
	else
	{
        ...
        if (uiModeKey == KEYSCAN_CUSTOMER_RELEASED)
        {
            ...
            //if((uiModeKeyPressCount)&&(uiModeKeyPressCount<KEYSCAN_CUSTOMER_COUNT))
            {	   
                uiModeKeyPressCount = 0;   
                UISound_Play(DEMOSOUND_SOUND_KEY_TONE);		
	...
	//eric edit begin
	//if((uiADCValue>=(KEY_ADC_OK-KEY_ADC_RANGE))&&(uiADCValue<(KEY_ADC_OK+KEY_ADC_RANGE)))
	if((uiADCValue>=(KEY_ADC_OK))&&(uiADCValue<(KEY_ADC_OK+KEY_ADC_RANGE)))
	{
        // Debounce
        if (uiOkKey == KEYSCAN_CUSTOMER_PRESSED)
        {
            if (uiOkState == KEYSCAN_CUSTOMER_RELEASE_STATE)
            {
                uiOkKeyPressCount++;
                debug_err(("uModeKeyPressCount : %d....%d\r\n",uiModeKeyPressCount,KEYSCAN_CUSTOMER_COUNT));
				
                if (uiOkKeyPressCount > KEYSCAN_CUSTOMER_COUNT)
                {
                    uiOkState = KEYSCAN_CUSTOMER_PRESS_STATE;
                }
            }

            if(uiOkState == KEYSCAN_CUSTOMER_PRESS_STATE)
            {
                  //uiKeyCode = FLGKEY_KEY_POWER;
		     uiOkKeyPressCount = 0;		
		     uiOkKey    = KEYSCAN_CUSTOMER_UNKNOWN;
		     uiOkState    = KEYSCAN_CUSTOMER_INIT_STATE;			
               // UISound_Play(DEMOSOUND_SOUND_KEY_TONE);					
                Ux_PostEvent(NVTEVT_KEY_SHUTTER2, 1, NVTEVT_KEY_CONTINUE);	
        ...
        {
            uiOkKey = KEYSCAN_PWRKEY_PRESSED;
        }
    }
	else
	{
        // Debounce
        if (uiOkKey == KEYSCAN_CUSTOMER_RELEASED)
        {
            if (uiOkState == KEYSCAN_CUSTOMER_INIT_STATE)
            {
                uiOkState = KEYSCAN_CUSTOMER_RELEASE_STATE;
            }

            if (uiOkKeyPressCount)
            //if ((uiModeKeyPressCount)&&(uiModeKeyPressCount<KEYSCAN_CUSTOMER_COUNT))
            {	   
                uiOkKeyPressCount = 0;   
                //UISound_Play(DEMOSOUND_SOUND_KEY_TONE);		
        ...
        {
            uiOkKey = KEYSCAN_CUSTOMER_RELEASED;
        }
    }
	 //eric edit end
}
