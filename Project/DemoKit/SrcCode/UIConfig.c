...
BOOL UI_ShowLogoFile(UINT32 logoId,UINT32 *uiJpgAddr,UINT32 *uiJpgSize)
{
    ...
    uiCurrentLogoID=logoId;
    psecnameId = PsBGLogoIDIndex[logoId];
    debug_err(("Section PS_BG_LOGO ID: %s ...\r\n",psecnameId));
    ...
    //show logo
    UI_ShowJPG(LOGO_DISP_LAYER, (UINT32)(BgBuf+4), uiFileSize, uiPoolAddr);
    return TRUE;
}
...
void UI_UpdateLogoFile(void)
{
    ...
    BOOL      ret, ret2;
    UINT32 i;
    PSFMT gFmtStruct = {512,64};
    UINT32 result=0;
    INT8 * update_ok = "Update logo  OK!";
    INT8 * update_fail = "Update logo  fail!";
    ...
    if (FileSys_OpenFile(fileName[0], FST_OPEN_READ) == NULL)
    {
        debug_err(("open file error\r\n"));
        return FALSE;
    }
        ...
        Cal_FillRect(&Rect, _OSD_INDEX_BLACK);
        Cal_ShowStringByColor("Update logo.....!\n", &Rect, _OSD_INDEX_YELLOW, _OSD_INDEX_BLACK);	
	for(i=0;i<FILE_BGLOGO_NUM;i++)
      {
	    	ret  = UI_WriteLogoFileName(fileName[i],PsBGLogoIDIndex[i]);
	      if (ret == FALSE)
	      	{
	      	    sprintf(update_fail, "%s  %d Fail!\n","Update logo",i+1);
	      	    Cal_FillRect(&Rect, _OSD_INDEX_BLACK);
                    Cal_ShowStringByColor(update_fail, &Rect, _OSD_INDEX_YELLOW, _OSD_INDEX_BLACK);
			//debug_msg("PS write file error: %s ,%s...\r\n",fileName[i],PsBGLogoIDIndex[i]);
		}
          else
            {
            sprintf(update_ok, "%s  %d OK!\n","Update logo",i+1);
            Cal_FillRect(&Rect, _OSD_INDEX_BLACK);
            Cal_ShowStringByColor(update_ok, &Rect, _OSD_INDEX_YELLOW, _OSD_INDEX_BLACK);
            }
	}
#if 0			
    ret  = UI_WriteLogoFileName(UI_LOGO_POWERON);
...
UINT32 UI_GetShowLogoID(void)
{
	return uiCurrentLogoID;
}
